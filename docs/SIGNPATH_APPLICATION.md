# SignPath Foundation 신청서 초안

이 문서는 SignPath Foundation 신청서의 공개 프로젝트 정보를 정확하고 일관되게 입력하기 위한 초안이다. 유지관리자의 실명, 이메일, 약관 동의 및 개인정보 처리 동의는 저장소에 기록하지 않으며 유지관리자가 신청 화면에서 직접 입력하고 확인한다.
