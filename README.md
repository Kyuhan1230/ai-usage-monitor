...
Codex CLI와 Claude Code 사용량을 한 화면에서 확인하는 로컬 Windows 트레이 앱이다.
...
기존 Electron 앱은 비교 및 백업 용도로 남아 있다.
...
친구나 팀원에게 전달할 때는 보통 `dist\Codex Claude Usage Setup.exe` 하나를 주면 된다. `dist\native` 폴더는 개발 중 portable 실행 확인용에 가깝다.
...
소스 파일만 수정한 경우 이미 설치된 앱에는 자동 반영되지 않는다. 설치본에 반영하려면 `npm run dist`로 Setup을 다시 만들고 재설치하거나, 개발 중에는 `npm run app`으로 저장소의 스크립트를 직접 실행한다.
...
scripts/
  native-usage-tray.ps1      WinForms tray 앱
  build-native-exe.ps1       native launcher exe 생성
  build-native-installer.ps1 NSIS setup 생성
...
src/electron/
  main.js                    legacy Electron 앱
  preload.js                 legacy IPC bridge
  renderer/                  legacy renderer UI
...
- 설치본을 사용 중이면 최신 Setup으로 다시 설치하거나 설치 폴더의 `scripts\native-usage-tray.ps1`이 최신인지 확인한다.
- 개발 중이면 트레이 앱을 완전히 종료한 뒤 `npm run app`으로 다시 실행한다.
