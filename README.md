...
| 고갈 예측 | 관측된 소진 속도, 예상 고갈 시각, reset 전 고갈 여부와 신뢰도 표시 |
| 로컬 알림 | 잔여 25% 주의·10% 위험 임계치, 이상 급증, reset 전 고갈 예측을 Windows 알림으로 안내 |
| 비교·비용 | 오늘/전일, 최근 7일/이전 7일 비교와 공식 API 정가 기준 비용 등가 추정 |
| 실행 가능한 추천 | 필요한 감속 비율, 반복 작업 점검, 저비용 모델 전환 시 절약 가능성을 규칙 기반으로 제안 |
...
- 비용은 실제 구독 청구액이 아니라 [OpenAI](https://platform.openai.com/docs/pricing)와 [Anthropic](https://platform.claude.com/docs/en/about-claude/pricing)의 2026-07-18 표준 API 정가로 환산한 참고치입니다. 가격을 알 수 없는 모델은 계산에서 제외합니다.
