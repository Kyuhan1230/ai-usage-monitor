...
3. 앱을 처음 실행하면 Setup이 자동으로 열려 두 CLI의 설치·로그인 상태를 직접 확인합니다. 필요한 도구에만 **설치** 또는 **로그인** 버튼이 표시됩니다.
...
4. 양쪽 로그인을 마친 뒤 **상태 다시 확인 → 설정 완료**를 누릅니다. Claude statusLine 이벤트 연결은 선택 사항입니다.
5. **사용량 확인**을 누르면 원샷으로 최신 상태를 읽습니다. `X`는 창과 WebView를 닫고 트레이만 남기며, 완전히 종료하려면 트레이 메뉴의 **Quit**을 선택합니다.
