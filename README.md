...
- 앱은 시작 15초 뒤 자동 확인을 시작하고, 켜져 있는 동안에도 다음 확인 시각이 되면 다시 확인합니다. 최신 버전이면 조용히 넘어가고, 새 버전이면 Windows 알림을 한 번 표시한 뒤 트레이의 **vX.Y.Z 업데이트 가능** 항목을 유지합니다. 알림을 놓쳐도 트레이 항목을 누르면 업데이트 창을 열 수 있습니다.
...
> [!WARNING]
> 업데이트 파일 검증 서명과 Windows 게시자 서명은 서로 다릅니다. 앱 안에서 받는 업데이트는 Tauri 업데이트 서명으로 파일이 공식 릴리스와 일치하는지 검증합니다. 현재 공개 설치 파일의 Windows Authenticode 게시자 서명은 SignPath Foundation 심사 중이므로, 승인 전에는 SmartScreen에 `알 수 없는 게시자`가 표시될 수 있습니다.
...
<table>
  ...
  <tr>
    <td align="center"><img src="docs/images/app-compact.png" alt="Codex Claude Usage 간단 보기 화면"></td>
    <td align="center"><img src="docs/images/app-setup.png" alt="Codex Claude Usage 설정 화면"></td>
  </tr>
  <tr>
...
| 사용자 승인 업데이트 | 앱을 켜 둔 동안에도 새 버전을 주기적으로 확인하고 Windows 알림·트레이에 표시하며, 사용자가 동의한 경우에만 검증된 설치 파일을 내려받아 설치·재시작 |
...
- 업데이트 확인에 성공하면 24시간 뒤 다시 확인합니다. 네트워크 연결에 실패하면 15분·1시간·6시간 간격으로 제한해 재시도하며, 설정 화면이나 트레이 메뉴에서는 언제든 직접 확인할 수 있습니다. 자동 확인은 업데이트 창을 강제로 띄우지 않습니다.
...
자세한 내용은 [개인정보 처리 방침](docs/PRIVACY.md), [보안 정책](SECURITY.md), [코드 서명 정책](docs/CODE_SIGNING_POLICY.md)을 확인하세요.
...
- 현재 공개 설치 파일은 SignPath Foundation 승인 전까지 Windows Authenticode 게시자 서명이 없습니다. 앱 안에서 받는 업데이트 파일의 Tauri 검증 서명과는 별개입니다.
...
제품 방향과 v1 공개 체크리스트는 [출시 계획](docs/LAUNCH_PLAN.md)에 기록했습니다.

## 라이선스

[MIT License](LICENSE) · Copyright © 2026 kyuhan1230

코드 서명은 [SignPath.io](https://signpath.io/)가 무료로 제공하며 인증서는 [SignPath Foundation](https://signpath.org/)에서 발급합니다. 현재 승인을 기다리고 있습니다.
