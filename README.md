...
- `codex_status_dashboard.py`: 플랜 잔여율(`/status` 캡처 결과)과 토큰 사용량 리포트를 하나로 합친 메인 대시보드.
- `codex_dashboard_fastapi.py`: 개발 중 파일 수정이 바로 반영되도록 FastAPI와 uvicorn reload로 대시보드를 띄우는 진입점.
- `codex_usage_report.py`: `~/.codex/sessions` JSONL을 스캔해서 날짜별, 모델별 토큰 사용량을 집계한다. 대시보드에서도 재사용하고, 독립적으로 정적 HTML을 뽑을 때도 쓴다.
- `codex-status-poller.js`: 대시보드가 자동으로 띄우는 헤드리스 백그라운드 프로세스. 숨겨진 Codex CLI 세션 하나를 잡아두고 주기적으로 `/status`를 캡처한다.
- `codex-wrapper.js` / `codex-wrapper.ps1`: 실제로 코딩할 때 쓰는 터미널을 감싸서, 그 세션의 실제 출력에서 `/status`를 캡처하는 보조 수단(선택 사항).
...
- 서버가 시작하면서 백그라운드에 숨겨진 Codex CLI 세션을 하나 띄우고(`codex-status-poller.js`), 기본 3분마다 `/status`를 캡처해서 `status.json`을 갱신한다. 사용자가 이 프로세스를 직접 신경 쓸 필요는 없다.
...
```powershell
python codex_status_dashboard.py --serve --no-auto-status-poll
```

캡처 주기, Codex 실행 파일, 세션 폴더 등은 옵션으로 바꿀 수 있다.

```powershell
python codex_status_dashboard.py --serve `
...
$shortcut.Arguments = '-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "D:\1. 프로젝트\스터디\CodexUsage\codex_status_dashboard_start.ps1"'
$shortcut.WorkingDirectory = "D:\1. 프로젝트\스터디\CodexUsage"
$shortcut.WindowStyle = 7
$shortcut.Save()
```

`codex_status_dashboard_start.ps1`이 `npm install`(필요할 때만) 후 대시보드를 창 없이 백그라운드로 띄우고, 출력을 `~\.codex-usage-wrapper\dashboard.log` / `dashboard-error.log`에 남긴다.

스크립트는 `http://127.0.0.1:8767/status.json`이 이미 응답하면 새 대시보드를 중복 실행하지 않고 종료한다.
...
$action = New-ScheduledTaskAction `
  ...
  -Argument '-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "D:\1. 프로젝트\스터디\CodexUsage\codex_status_dashboard_start.ps1"'
...
```powershell
.\codex-wrapper.ps1 --no-after-output-capture
```

자동 시작 캡처를 끄려면:

```powershell
.\codex-wrapper.ps1 --no-start-capture
```

idle 캡처를 끄려면:

```powershell
.\codex-wrapper.ps1 --no-idle-capture
```

Codex 인자를 넘기려면 `--` 뒤에 쓴다.

```powershell
.\codex-wrapper.ps1 -- --model gpt-5.5
```
...
```powershell
cd "D:\1. 프로젝트\스터디\CodexUsage"
Get-Clipboard | python codex_status_dashboard.py --raw-stdin
```
...
```powershell
python codex_usage_report.py
```

실시간 재스캔 서버:

```powershell
python codex_usage_report.py --serve
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node \"D:\\1. 프로젝트\\스터디\\CodexUsage\\claude-status-hook.js\""
  }
}
```
...
node "D:\1. 프로젝트\스터디\CodexUsage\claude-status-hook.js"
```
