...
| 영역 | 제공 기능 |
| --- | --- |
| 한도 모니터링 | Codex 5-hour/weekly, Claude current session/current week 잔여율과 Compact의 시간당 소진 속도 |
| 고갈 예측 | 30분·2시간 단위로 다듬은 최근 평균 속도, 빠른/느린 고갈 예상, reset 전 소진 여부와 근거 |
| 로컬 알림 | 잔여 25% 주의·10% 위험, 이상 급증, reset 전 고갈 예측 |
| 비교·비용 | 오늘/전일, 최근 7일/이전 7일 비교와 API 정가 기준 비용 등가 추정 |
| 실행 가능한 추천 | 리셋까지 필요한 감속 비율, 반복 작업 점검, 저비용 모델 전환 가능성을 규칙 기반으로 제안 |
...
| 사용자 승인 업데이트 | 시작 15초 뒤 GitHub Release를 확인하고, 성공한 확인 뒤 24시간 동안 자동 재확인을 생략하며, 사용자가 동의한 경우에만 서명된 설치 파일을 내려받아 설치·재시작 |
...
1. [Latest Release](https://github.com/Kyuhan1230/ai-usage-monitor/releases/latest)에서 `Codex-Claude-Usage-Setup-<version>.exe`를 설치합니다.
   - updater가 처음 포함되는 `1.2.0`은 기존 `1.1.1`이 감지할 수 없어 한 번 직접 설치해야 합니다.
   - `1.2.0` 이후 버전은 앱의 **새 버전이 있습니다** 창 또는 Setup·트레이의 **업데이트 확인**에서 설치할 수 있습니다.
...
설치본에 Codex CLI나 Claude Code를 포함하지 않아 설치 파일 크기를 키우지 않으며, 별도 Node.js나 Python도 필요하지 않습니다. 앱은 새 버전 확인을 위해 GitHub Release의 `latest.json`만 조회하고, 업데이트 설치는 사용자가 **업데이트**를 누른 경우에만 시작합니다. 대화형 설치 또는 Setup에서 사용자가 특정 CLI 설치에 동의한 경우에만 해당 공급자의 공식 설치 스크립트로 별도 네트워크 요청을 보냅니다. 앱과 설치 프로그램은 WebView2를 자동 다운로드하지 않으므로, WebView2가 제거된 Windows 환경에서는 [Microsoft Edge WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)을 사용자가 먼저 설치해야 합니다.
...
- 앱 자체의 텔레메트리·원격 분석 요청은 없습니다. 업데이트 확인에 성공하면 24시간 동안 자동 재확인하지 않으며, 네트워크 실패 시 다음 앱 시작에서 다시 시도할 수 있습니다. Setup·트레이에서는 사용자가 언제든 수동으로 확인할 수 있습니다. 수동 새로고침과 사용자가 켠 활동 기반 자동 확인의 원샷 CLI 통신·인증은 각 CLI가 관리합니다.
...
Tauri NSIS 설치 파일은 `src-tauri/target/release/bundle/nsis/`에 만들어집니다. 일반 CI는 updater artifact를 만들지 않는 overlay로 무서명 설치 파일을 검증하고, Release는 GitHub secret의 Tauri 개인키로 `.exe.sig`와 `latest.json`을 생성합니다. 두 workflow 모두 20MB를 넘는 설치 파일을 실패 처리합니다.
...
scripts/                  릴리스 태그·updater manifest 생성 및 검증 도구
